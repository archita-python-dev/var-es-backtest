...
Generated from `outputs/` by `python -m var_backtest.compare --out comparison2`.
Models compared: Historical, Parametric (Normal), Parametric (t + EWMA), Monte Carlo (Student-t).
...
Forecast Expected Shortfall against the loss actually suffered on each breach day.
